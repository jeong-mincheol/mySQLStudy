...
SELECT a.productCode, a.productName, b.productLine, b.textDescription
FROM products a
INNER JOIN productlines b ON a.productLine = b.productLine;
...
SELECT a.employeeNumber, a.lastName, a.firstName, b.phone FROM employees a
...
-- 일별 매출액 조회
SELECT a.orderDate, SUM(b.quantityOrdered * b.priceEach)
from orders a
...
LEFT JOIN orderdetails b ON a.orderNumber = b.orderNumber
GROUP BY a.orderDate
HAVING a.orderDate <= '2003-01-31';


SELECT SUBSTRING(orderDate, 1, 4), orderdate 
FROM orders
WHERE SUBSTRING(orderDate, 1, 4) = '2004';


SELECT SUBSTRING(a.orderDate, 1, 4) years, SUBSTRING(a.orderDate, 6, 2) months, SUM(b.quantityOrdered * b.priceEach) total
FROM orders a
...
-- 인당 매출액(연도별)
-- 연도, 고객수, 매출액
SELECT SUBSTRING(a.orderDate, 1, 4) years, 
		 COUNT(DISTINCT(customernumber)) customCnt, 
		 SUM(b.quantityOrdered * b.priceEach) total,
		 SUM(b.quantityOrdered * b.priceEach) / COUNT(DISTINCT(customernumber)) avgprice
FROM orders a
LEFT JOIN orderdetails b
ON a.orderNumber = b.orderNumber
GROUP BY years
...
SELECT C.country, C.city, SUM(B.quantityOrdered * B.priceEach) 
FROM orders A 
LEFT JOIN orderdetails B 
ON A.orderNumber = B.orderNumber
...
SELECT case when country IN ('USA', 'Canada') then '북미' 
			ELSE '기타'
		 END 지역, 
...
SELECT * FROM sk17.naver_day_stock WHERE st_date = '2025-05-29';

SELECT st_code, st_date, st_open, st_high, st_low, st_close, st_close * st_trade 거래금액 FROM sk17.naver_day_stock WHERE st_date = '2025-05-29';

CREATE VIEW master_code AS
SELECT CONCAT('A',isu_srt_cd) st_code, isu_abbrv, list_dd, mkt_tp_nm, list_shrs FROM sk17.st_master ;

CREATE TABLE master_code2 AS
SELECT CONCAT('A',isu_srt_cd) st_code, isu_abbrv, list_dd, mkt_tp_nm, list_shrs FROM sk17.st_master ;

SELECT A.st_code, A.st_date, A.st_open, A.st_high, A.st_low, A.st_close, A.st_close * A.st_trade 거래금액, B.isu_abbrv
FROM sk17.naver_day_stock A
INNER JOIN sk17.master_code2 B
...
-- rank 는 점수가 같을 경우 1,1,1,4등 이렇게 나온다
-- dense_rank는 점수가 같을 경우 1,1,1,4등 이렇게 나온다
-- row_number는 1,2,3,4
...
SELECT 
  C.country, 
  SUM(B.quantityOrdered * B.priceEach) AS 판매금액,
  RANK() OVER (ORDER BY SUM(B.quantityOrdered * B.priceEach) DESC) AS 랭킹
FROM orders A 
LEFT JOIN orderdetails B ON A.orderNumber = B.orderNumber
...
-- 고객별 마지막 구매일자
SELECT customernumber, max_date, '2005-06-01', DATEDIFF('2005-06-01', max_date)
FROM 
...
FROM
(SELECT *, case when diff >= 90 then '위험'
			 ELSE '정상'
			 END customer_type
FROM (
